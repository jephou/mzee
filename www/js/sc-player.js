...
var SoundCloudPlayer = function(tracks, config){
	...
	this.play = function(){
		...
		if( _this.sound && _this.sound.readyState == 1 ){
			
			_this.sound.play();
			//$('#dialog_title_span').append("<hr><hr>"+_this.sound.toSource()+"<hr><hr>"); 
			//$('#dialog_title_span').apend(_this.sound.play()); 
		}else{
	...
	this.next = function(autoplay){
		_this.log('next');
		//play the next track?
		_this.play_when_ready = false;//(typeof autoplay != 'undefined')? autoplay : _this.config.autoswitch;
	...
	_this.resolve_track = function(url, cb){
		...
		*/
		...
		jQuery.ajax({
			  ...
			  url: ajax_url_for_player+url
			, dataType: 'jsonp'
			, error: function(jqXHR, textStatus, errorThrown){
				promise.reject(jqXHR, textStatus, errorThrown);
			}
			, success: function(_track){ //alert(url);
				
				//$('#dialog_title_span').append(_track.toSource()+"<hr>"); 
